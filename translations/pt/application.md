...
| TemplateEngine            | `func(raw string, bind interface{}) (string, error)` | Você pode especificar um template personalizado através de um método para renderizar em outros idiomas. Veja nosso [**Template Middleware**](middleware.md#template) para predefinições.                                                                                  | `nil`             |
| TemplateFolder            | `string`                                             | Um diretório para as views (templastes) da aplicação. Se um diretório for definido, isso será o prefixo para todos os caminhos dos templates. `c.Render("home", data) -> ./views/home.pug`                                                                             | `""`              |
| TemplateExtension         | `string`                                             | Se você predefinir o arquivo da extensão do template, você não precisa colocar o nome completo do arquivo na função que renderiza: `c.Render("home", data) -> home.pug`                                                                                                | `"html"`          |

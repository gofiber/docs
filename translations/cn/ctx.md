...
基于请求的 [Accept](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Accept) HTTP 头字段作出对应的内容处理。 使用 [Accepts](ctx.md#accepts) 选择一个适当的格式。
...
使用 JSONP 发送JSON 响应。 这个方法与 [JSON](ctx.md#json) 完全相同，只是加入了支持 JSONP 的回调。 默认情况下，回调名称是 callback。
